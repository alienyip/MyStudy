...
public class AutoScrollBackLayout extends FrameLayout {
    ...
    public AutoScrollBackLayout(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mHideAnimation = AnimationUtils.loadAnimation(context, hideAnimResourceId);
    ...
    private void hookScrollListenerForAbsListview() {
        try {
            ...
            if (DEBUG) {
                Log.i(TAG, "target=" + target.getClass().getName() + " ,proxy=" + proxy.getClass().getName() + ",proxied interface=" + Arrays.toString(proxy.getClass().getInterfaces()));
            }
    ...
    private class MyScrollLitener {
        ...
        void onScrollStateChanged(View scrollview, int scrollState) {
            ...
            if (DEBUG) {
                Log.d(TAG, logTarget + " scrollState=" + scrollState + ",scrollDistance=" + scrollDistance + ",showScrollDistance");
            }
            if (scrollState == 0) {
                if (DEBUG) {
    ...
    private int getScrollDistance(int dy) {
        ...
        if (wrapView instanceof  AbsListView) {
            ...
            if (layoutManager instanceof LinearLayoutManager) {
                ...
                return -top + firstVisibleItemPosition * firstChild.getHeight();
            }
            return dy;
        }
        return 0;
