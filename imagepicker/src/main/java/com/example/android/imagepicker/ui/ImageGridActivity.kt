...
import com.example.android.imagepicker.*
import com.example.android.imagepicker.adapter.ImageFolderAdapter
import com.example.android.imagepicker.adapter.ImageRecyclerAdapter
import com.example.android.imagepicker.bean.ImageFolder
import com.example.android.imagepicker.bean.ImageItem
import com.example.android.imagepicker.util.CameraUtil
import com.example.android.imagepicker.util.Utils
import com.example.android.imagepicker.util.isSameDate
import com.example.android.imagepicker.view.FolderPopupWindow
...
class ImageGridActivity : AppCompatActivity() ,ImageRecyclerAdapter.OnImageItemClickListener, ImageDataSource.OnImagesLoadedListener, View.OnClickListener{
        ...
        recycler.addOnScrollListener(object : RecyclerView.OnScrollListener(){
            override fun onScrollStateChanged(recyclerView: RecyclerView?, newState: Int) {
                if (newState == RecyclerView.SCROLL_STATE_IDLE) {
                    ...
                    Log.d("example", "图片, position: $position , addTime: $addTime")
                    if (addTime != null) {
    ...
    override fun onImageItemClick(imageItem: ImageItem, position: Int) {
        if (pickerHelper.isMultiMode) {
            ...
            var p = position
            if (images.size > INTENT_MAX) {//数量过大
                val s: Int
                val e: Int
                ...
                p = position - s
                Log.e("example", "start:$s, end:$e, postion:$p")
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == REQUEST_CAMERA) {//相机返回
            if (requestCode == Activity.RESULT_OK) {
                Log.e("example", takeImageFile.absolutePath)
                //广播通知新增图片
